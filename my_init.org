...
By default on my Japanese MacBook Air keyboard (JIS), command is super (s), option and esc are meta (M), control is control (C). fn does nothing. I leave the Japanese language keys alone. I add 



Using karabiner-elements I flip control (left of a) and caps (left of left option). The new caps key sends C-M-s when combined with another key and <f18> when hit alone. The underscore key to the left of the right shift I change to provide ’\’ unshifted, ’_’ shifted in Keyboard Maestro. Emacs sees the first as “M-¥”, so the remapping of it to send a backslash.
...
** org-wunderlist                                          :private:disabled:
